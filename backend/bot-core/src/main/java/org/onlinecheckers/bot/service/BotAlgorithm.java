 ...
 * Contains the minimax algorithm with alpha-beta pruning and anti-repetition logic.
 */
public class BotAlgorithm {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                ...
                for (int[] dir : directions) {
                    ...
                    String[][] boardCopy = copyBoard(board);
                    boardCopy[landRow][landCol] = boardCopy[i][j];
                    boardCopy[i][j] = "";
                    boardCopy[captureRow][captureCol] = "";
    ...
    private void findMultipleCaptures(String[][] board, Team team, int row, int col, boolean isKing,
        ...
        for (int[] dir : directions) {
            ...
            String[][] boardCopy = copyBoard(board);
            boardCopy[landRow][landCol] = boardCopy[row][col];
            boardCopy[row][col] = "";
            boardCopy[captureRow][captureCol] = "";
