...
import { OfflineBoardComponent } from '../offline-board/offline-board';
import { OfflineMovesComponent } from '../offline-moves/offline-moves';
import { InfoModalComponent } from '../modal/info-modal';
...
@Component({
  ...
  templateUrl: './bot-board.html',
  styleUrl:    './bot-board.css'
})
export class BotBoardComponent extends OfflineBoardComponent implements OnInit, OnDestroy {
  ...
  private botSubscription: any = null;
  ...
  override ngOnInit() {
    ...
    const hasSeenOfflineInfo = localStorage.getItem('botIntroShown');    
    if (hasSeenOfflineInfo) this.showInfoModal = false;
  ...
  override onConfirmModal(): void {
    if (!isPlatformBrowser(this.platformId)) return;

    this.showInfoModal = false;
    localStorage.setItem('botIntroShown', 'true');
  }
  ...
  getBotMove() {
    ...
    // Cancel any pending bot request
    if (this.botSubscription) this.botSubscription.unsubscribe();

    this.botSubscription = this.botService.calculateMove(request).subscribe({
      next: (response) => {
        // Check if subscription is still valid (game not reset)
        if (!this.botSubscription) return console.log('Bot move ignored - game was reset');
        ...
        if (response.path && response.path.length > 0) {
          ...
          // Simple move
          super.makeMove(fromRow, fromCol, toRow, toCol);
          this.currentPlayer = this.playerColor; // Force player turn after bot move
        }
