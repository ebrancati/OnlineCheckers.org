...
import { NgClass, isPlatformBrowser, DOCUMENT } from '@angular/common';
import { Router, ActivatedRoute } from '@angular/router';
import { interval, Subscription } from 'rxjs';

import { OnlineMovesComponent as MovesComponent } from '../online-moves/online-moves';
...
@Component({
  ...
  templateUrl: './online-board.html',
  styleUrl:    './online-board.css',
})
export class OnlineBoardComponent implements OnInit, OnDestroy {
  ...
  whitePlayerNickname: string = 'Awaiting opponent...';
  blackPlayerNickname: string = 'Awaiting opponent...';
  ...
  private optimisticMoves: Set<string> = new Set();
  ...
  ngOnDestroy() {
    ...
    if (this.restartPollingSubscription) this.restartPollingSubscription.unsubscribe();
    if (this.captureAnimationInterval)   clearInterval(this.captureAnimationInterval);
  }
  ...
  private connectToWebSocket(): void {
    ...
    this.webSocketService.connect(this.gameID, this.nickname);
    ...
    this.connectionStatusSubscription = this.webSocketService.connectionStatus$.subscribe({
      next: (status) => {
        ...
        if (status === 'connected') setTimeout(() => { this.showConnectionStatus = false }, 3000);
        else this.showConnectionStatus = true;
      }
    });
    ...
    this.gameService.getGameState(this.gameID).subscribe({
      next: (gameAccess: GameAccessDto) => {
        ...
        if (this.isSpectator) {
          ...
          if (this.nickname) {
            const playerMatch = gameState.players.find((player: { nickname: string; }) => player.nickname === this.nickname);
            if (playerMatch) this.playerTeam = playerMatch.team as 'WHITE' | 'BLACK';
          }
        }
  ...
  private processGameStateUpdate(gameState: any, authorizedGameState?: any): void {
    ...
    for (const player of gameState.players) {
      if      (player.team === 'WHITE') this.whitePlayerNickname = player.nickname;
      else if (player.team === 'BLACK') this.blackPlayerNickname = player.nickname;
    }
  ...
  private handleRestartStatusUpdate(restartStatus: any): void {
    ...
    if      (this.playerTeam === 'WHITE' && restartStatus.restartW) this.waitingForOpponentRestart = true;
    else if (this.playerTeam === 'BLACK' && restartStatus.restartB) this.waitingForOpponentRestart = true;
  ...
  private handleBothPlayersWantRestart(): void {
    ...
    if (this.nickname)
      this.webSocketService.resetGame(this.gameID, this.nickname);
  ...
  makeMove(fromRow: number, fromCol: number, toRow: number, toCol: number): void {
    ...
    const moveId = this.generateMoveId(optimisticMove);
    this.moves.push(optimisticMove);
    this.optimisticMoves.add(moveId);
    ...
    if (isCapture && further.length > 0) {
      ...
      if (this.nickname) {
        ...
        this.webSocketService.makeMove(this.gameID, this.nickname, moveData);
      }
   ...
   */
  private generateMoveId(move: Move): string {
    const captureInfo = move.captured ? move.captured.map(c => `${c.row}${c.col}`).join(',') : '';
    return `${move.from.row}${move.from.col}-${move.to.row}${move.to.col}-${captureInfo}`;
  }

  /**
   * Remove duplicate optimistic moves when server confirms them
   */
  private removeDuplicateOptimisticMoves(): void {
    // Convert current moves to IDs for comparison
    const currentMoveIds = this.moves.map(move => this.generateMoveId(move));
    
    // Remove optimistic moves that are now confirmed by server
    this.optimisticMoves.forEach(optimisticId => {
      if (currentMoveIds.includes(optimisticId)) {
        this.optimisticMoves.delete(optimisticId);
      }
    });
  }
  ...
  requestRestart(): void {
    ...
    if (this.nickname)
      this.webSocketService.updateRestartStatus(this.gameID, this.nickname, updatedStatus);
    ...
    this.gameService.getGameState(this.gameID).subscribe({
      next: (gameAccess: GameAccessDto) => {
        ...
        if (this.nickname) {
          ...
          const playerMatch = response.players.find(player => player.nickname === this.nickname);
          if (playerMatch)
            this.playerTeam = playerMatch.team as 'WHITE' | 'BLACK';
          else
            console.log(`Nessun giocatore con nickname ${this.nickname} trovato nei giocatori della partita:`, response.players);
        }
        else {
  ...
  updateMovesFromHistory(moveHistory: string[]): void {
    ...
    // Extract only optimistic moves before doing anything
    const optimisticMoves = this.moves.filter(move => {
      const moveId = this.generateMoveId(move);
      return this.optimisticMoves.has(moveId);
    });

    console.log('Current optimistic moves:', optimisticMoves.length);
    console.log('Server move history length:', moveHistory.length);
    
    // Rebuild confirmed moves from server history
    const confirmedMoves: Move[] = [];
    ...
    for (const moveString of moveHistory) {
      ...
      // is it a multiple capture?
      if (isCapture && (fromPos === lastFromPos || (confirmedMoves.length > 0 && fromPos === confirmedMoves[confirmedMoves.length - 1].to.row + '' + confirmedMoves[confirmedMoves.length - 1].to.col))) {
          // Add to existing multi-capture
          const lastMove = confirmedMoves[confirmedMoves.length - 1];
          ...
          confirmedMoves.push(move);
      }

      lastFromPos = fromPos;
    }

    // Remove optimistic moves that are now confirmed
    const confirmedMoveIds = confirmedMoves.map(move => this.generateMoveId(move));
    const stillOptimisticMoves = optimisticMoves.filter(move => {
      const moveId = this.generateMoveId(move);
      const isConfirmed = confirmedMoveIds.includes(moveId);
      if (isConfirmed) {
        this.optimisticMoves.delete(moveId);
        console.log('Removing confirmed optimistic move:', moveId);
      }
      return !isConfirmed;
    });

    // Combine confirmed moves with remaining optimistic moves
    this.moves = [...confirmedMoves, ...stillOptimisticMoves];
    
    console.log('Final moves array length:', this.moves.length);
    console.log('Remaining optimistic moves:', stillOptimisticMoves.length);
  }
  ...
  updateGameState(response: GameResponse) {
    ...
    if (this.gameOver && response.vincitore !== 'NONE') {
      ...
      if (this.playerTeam === response.vincitore) this.audioService.playWinSound();
      else                                        this.audioService.playLoseSound();
  ...
  */
  needsOpponent(): boolean {
    ...
    return !this.whitePlayerNickname || !this.blackPlayerNickname ||
           this.whitePlayerNickname === 'Awaiting opponent...' ||
  ...
  */
  fetchRestartStatus() {
    ...
    this.restartService.getRestartStatus(this.gameID).subscribe({
        ...
        if      (this.playerTeam === 'WHITE' && status.restartW) this.waitingForOpponentRestart = true;
        else if (this.playerTeam === 'BLACK' && status.restartB) this.waitingForOpponentRestart = true;
  ...
  */
  hasOpponentRequestedRestart(): boolean {
    ...
    if      (this.playerTeam === 'WHITE') return this.restartStatus.restartB;
    else if (this.playerTeam === 'BLACK') return this.restartStatus.restartW;

    return false;
  }

  resetGame(): void {
    if (this.gameID) this.requestRestart();
